...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/saltext_zabbix/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/saltext_zabbix/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/saltext_zabbix/*"
            )
            ...
            #    "--include=src/saltext/saltext_zabbix/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/saltext_zabbix/config/schemas",
    )
