...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/zabbix/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/zabbix/*")
            ...
            #    "--include=src/saltext/zabbix/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/zabbix/config/schemas",
    )
