...
def present(name, mediatype, **kwargs):
    ...
    if mediatype_exists:
        ...
        if (
            ...
            if (
                int(kwargs["smtp_authentication"]) != int(mediatypeobj["smtp_authentication"])
        ...
        if (
            ...
            if update_email_security:
                updated_email_security = __salt__["zabbix.mediatype_update"](
                    mediatypeid, smtp_security=kwargs["smtp_security"], **connection_args
                )
            ...
            if update_email_verify_peer:
                updated_email_verify_peer = __salt__["zabbix.mediatype_update"](
                    mediatypeid, smtp_verify_peer=kwargs["smtp_verify_peer"], **connection_args
                )
            ...
            if update_email_verify_host:
                updated_email_verify_host = __salt__["zabbix.mediatype_update"](
                    mediatypeid, smtp_verify_host=kwargs["smtp_verify_host"], **connection_args
                )
            ...
            if update_script:
                updated_script = __salt__["zabbix.mediatype_update"](
                    mediatypeid, type=mediatype, exec_path=kwargs["exec_path"], **connection_args
                )
            ...
            if update_sms:
                updated_sms = __salt__["zabbix.mediatype_update"](
                    mediatypeid, type=mediatype, gsm_modem=kwargs["gsm_modem"], **connection_args
                )
