...
def __virtual__():
    cfg_key = f"{__virtualname__}.sender_bin"
...
def _get_config(ret):
    ...
    if os.path.exists("/etc/zabbix/zabbix_agent2.conf"):
        defaults["config"] = "/etc/zabbix/zabbix_agent2.conf"
    elif os.path.exists("/etc/zabbix/zabbix_agentd.conf"):
...
def _zabbix_send(exe, options):
    ...
    for option, value in options.items():
        ...
        cmd += f" --{shlex.quote(option)} {shlex.quote(value)}"
    ...
    if result["retcode"] == 0:
        ...
        msg = f"zabbix_sender failed with retcode {result['retcode']}"
